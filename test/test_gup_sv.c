...
void test_gup_sv_create_from_cstr(void) {
    { // NULL gives empty string view
        GupStringView sv = gup_sv_create_from_cstr(NULL);

        assert(gup_cstr_eq(sv.data, ""));
    }

    { // Empty string gives empty string view
        GupStringView sv = gup_sv_create_from_cstr("");
    ...
    {
        GupStringView sv = gup_sv_create_from_cstr("Hello, world!");
...
void test_gup_sv_chop_by_sv(void) {
    { // Buffer gets the part before the delimiter
        GupStringView sv = gup_sv_create_from_cstr("Hello, world!");
        GupStringView delim = gup_sv_create_from_cstr(", ");
    ...
    { // Buffer gets the entire string if delim is not found
        GupStringView sv = gup_sv_create_from_cstr("Hello, world!");
        GupStringView delim = gup_sv_create_from_cstr("Goodbye");
        GupStringView buffer = gup_sv_chop_by_sv(&sv, delim);
